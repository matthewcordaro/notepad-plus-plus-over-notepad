...
# If Notepad plus plus isn't installed, it tries to install the package. Exits upon failure. 
if (!$(Test-Path $NotepadPlus))
{
	try { Install-Package -Name NotePadPlusPlus -Force }
	catch { throw "Failed to install NotepadPlusPlus Package."; exit }
}
